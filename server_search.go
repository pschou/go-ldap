...
func HandleSearchRequest(req *ber.Packet, bindSpec BindSpec, controls *[]Control, messageID int64, conn net.Conn) (resultErr error) {
	defer func() {
		if r := recover(); r != nil {
	...
	searchReq, err := parseSearchRequest(req, bindSpec, controls)
	if err != nil {
		bindSpec.Server.Debug.Println("Parse Search error", err)
		return NewError(LDAPResultOperationsError, err)
	}

	filterPacket, err := CompileFilter(searchReq.Filter)
	if err != nil {
		bindSpec.Server.Debug.Println("Compile Search error", err)
		return NewError(LDAPResultOperationsError, err)
	}

	fnNames := []string{}
	for k := range bindSpec.Server.SearchFns {
		fnNames = append(fnNames, k)
	}
	fn := routeFunc(searchReq.BaseDN, fnNames)
	searchResp, err := bindSpec.Server.SearchFns[fn].Search(bindSpec, searchReq, conn)
	if err != nil {
		bindSpec.Server.Debug.Println("Search error", err)
		return NewError(searchResp.ResultCode, err)
	}

	if bindSpec.Server.EnforceLDAP {
	...
	for _, entry := range searchResp.Entries {
		if bindSpec.Server.EnforceLDAP {
			...
			if len(searchReq.Attributes) > 1 || (len(searchReq.Attributes) == 1 && len(searchReq.Attributes[0]) > 0) {
				...
				if err != nil {
					bindSpec.Server.Debug.Println("Search attribute error", err)
					return NewError(LDAPResultOperationsError, err)
				}
			}
		...
		if err = sendPacket(conn, responsePacket); err != nil {
			bindSpec.Server.Debug.Println("Search encode response error", err)
			return NewError(LDAPResultOperationsError, err)
		}
	}
	return nil
...
/////////////////////////
func parseSearchRequest(req *ber.Packet, bindSpec BindSpec, controls *[]Control) (SearchRequest, error) {
